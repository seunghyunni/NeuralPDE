...
parser.add_argument('--save', type=str, default='./mob3_experiment3')
parser.add_argument('--dataset', type=str, default='tinyimagenet')
parser.add_argument('--debug', action='store_true')
parser.add_argument('--gpu', type=int, default=0)
parser.add_argument('--adjoint', type=eval, default=True, choices=[True, False])
parser.add_argument('--is_ode', type=eval, default=False, choices=[True, False])
...
if __name__ == '__main__':
    ...
    # Model
    if args.is_ode:
        model = ContinuousMobileNetV3().to(device)
    else:
    ...
    optimizer = torch.optim.RMSprop(model.parameters(), lr=args.lr, momentum=0.9, weight_decay= 1e-5)

    # Learning Rate Scheduler
    #lr_fn = StepLR(optimizer, step_size = 60, gamma =0.2, last_epoch = -1)
    ...
    for epoch in range(args.nepochs):
        ...
        if val_acc > best_acc:
            torch.save({'state_dict': model.state_dict(), 'args': args},
                       os.path.join(args.save, 'model_dict_%.4f_%d.pth' % (val_acc, epoch+1)))
            best_acc = val_acc
